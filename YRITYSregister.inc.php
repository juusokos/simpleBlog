    ...
    if (empty($error_msg)) {
        ...
        if ($insert_stmt = $mysqli->prepare("INSERT INTO simple_users (username, email, password, salt) VALUES (?, ?, ?, ?)")) {
            ...
            if (! $insert_stmt->execute()) {
				...
				$user_ID = $select->fetch();

				luoBlogi();
			}
        }
       // header('Location: luoBlogi.php?'.$user_ID.'');
...
/* Luodaan uusi sivu oletusteksteillä */
				...
				$luoBlogi->bind_param('iiisss', $user_ID, '1', '1', 'Hei! Tähän voit kirjoittaa kuvauksen itsestäsi tai blogistasi. Muokkaa tätä tekstiä Asetukset - välilehdellä.',
				'Uusi Simple Blogi',
				'Sivuston kuvaus tulee tähän halutessasi. Muokkaa kuvausta tai poista se Asetukset - välilehdellä.');
				$luoBlogi->execute();
				var_dump($luoBlogi);
